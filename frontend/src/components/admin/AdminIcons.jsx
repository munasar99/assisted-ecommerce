/** Merge size + color — className passed from parent must not replace h/w. */
const cn = (size, className) =>
  ["shrink-0 inline-block", size, className].filter(Boolean).join(" ");

const SIZE_MD = "h-5 w-5";
const SIZE_SM = "h-4 w-4";

export function IconOverview({ className }) {
  return (
    <svg className={cn(SIZE_MD, className)} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75} aria-hidden>
...
export function IconEdit({ className }) {
  return (
    <svg className={cn(SIZE_SM, className)} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden>
